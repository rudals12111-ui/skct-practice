import {uid,escapeHTML as esc,normalizeRect,gridRects,validateProject,DEFAULT_SECTIONS,EXAM_SPEC,orderQuestions,parseQuestionLabel,isAutoLabel,questionLabel} from './core.js';
...
<div class="builder-config"><label>문제집 이름 <input id="projectName" value="${esc(draft.name)}" maxlength="200"></label><label>기본 영역 <select id="cropSection">${DEFAULT_SECTIONS.map(s=>`<option value="${s.id}">${esc(s.name)}</option>`).join('')}</select></label><label title="직접 잘라 추가할 문항의 번호. 추가할 때마다 1씩 늘어납니다.">번호 <input id="cropNumber" type="number" min="1" max="300" placeholder="자동" style="width:64px"></label><label>편집 <select id="columnMode"><option value="auto">단 자동 판별</option><option value="1">1단</option><option value="2">2단 (왼쪽 → 오른쪽)</option></select></label></div>
...
</div><aside><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px"><strong id="draftCount">준비한 문항 0</strong><div style="display:flex;align-items:center;gap:6px"><label class="order-toggle" title="영역·번호를 고치면 시험 순서(영역 순, 번호 순)로 목록을 다시 배치합니다"><input type="checkbox" id="autoOrder" checked> 번호순 정렬</label><button id="undoDraft" title="마지막 문항 편집 되돌리기">↶ 되돌리기</button></div></div><div id="sectionCounts" class="section-counts"></div><div class="builder-list" id="builderList"></div><p class="inline-message">영역별로 묶어 시험에 표시합니다. 실제 시험은 영역마다 20문항입니다. 정답을 모르면 ‘미입력’으로 둡니다.</p><div style="display:flex;justify-content:space-between;align-items:center;gap:8px"><button id="keyFromBuilder" title="정답표 스크린샷이나 PDF 정답 쪽을 읽어 정답을 채웁니다">정답표로 정답 채우기</button><button id="clearDraft" class="text-btn danger">문항 목록 비우기</button></div></aside></div>
<div class="library-footer"><p>원본 파일 업로드 없음 · OCR도 이 기기에서 실행 · 편집 내용 자동 저장<br>현재 PDF: <span id="pdfFileName">없음</span></p><div><button data-close="libraryDialog">나중에 이어서</button><button id="applyProject" class="primary">시험에 적용</button></div></div>`;
 ...
 $('builderList').onchange=e=>onListChange(e);
 try{$('autoOrder').checked=localStorage.getItem('skct-auto-order')!=='0';}catch{}
 $('autoOrder').onchange=()=>{try{localStorage.setItem('skct-auto-order',$('autoOrder').checked?'1':'0');}catch{}if($('autoOrder').checked&&reorder(selectedId))status('영역·번호 순서로 다시 배치했습니다.');};
 $('applyProject').onclick=async()=>{if(!draft.questions.length||busy)return;if(reorder(selectedId))saveDraft();const counts=sectionCounts();const off=DEFAULT_SECTIONS.filter(s=>counts[s.id]&&counts[s.id]!==EXAM_SPEC.questionsPerSection).map(s=>`${s.name} ${counts[s.id]}`);if(!await ask('이 문제집으로 시험을 준비할까요?',`${draft.questions.length}문항을 적용하고 기존 시험 답안과 시간을 초기화합니다.${off.length?` 실제 시험(영역당 ${EXAM_SPEC.questionsPerSection}문항)과 문항 수가 다른 영역: ${off.join(', ')}.`:''}`,'시험에 적용'))return;draft.name=$('projectName').value||'내 PDF 문제집';await onApply({...clone(),id:uid()});$('libraryDialog').close();toast('문제집을 적용했습니다. 시험 설정을 확인하고 시작하세요.');};
 ...
 /** Re-sort the list into exam order (section order, then number). Returns true when anything moved. */
 function reorder(focusId){
  if(!$('autoOrder')?.checked)return false;
  const before=draft.questions.map(q=>q.id).join();
  draft.questions=orderQuestions(draft.questions);
  const moved=before!==draft.questions.map(q=>q.id).join();
  renderList();
  if(focusId)requestAnimationFrame(()=>$('builderList').querySelector(`[data-id="${CSS.escape(focusId)}"]`)?.scrollIntoView({block:'nearest'}));
  return moved;
 }
 function relabel(q,old){if(isAutoLabel(q.label)&&(q.number||old.number))q.label=questionLabel(q.section,q.number,draft.questions.indexOf(q));}
 async function onListChange(e){
  const q=draft.questions.find(q=>q.id===e.target.dataset.q);if(!q)return;
  const field=e.target.dataset.field,old={section:q.section,number:q.number};
  checkpoint();
  if(field==='key'){q.key=Number(e.target.value)||null;saveDraft();return;}
  if(field==='section')q.section=e.target.value;
  if(field==='number'){const n=Math.round(Number(e.target.value));q.number=n>=1&&n<=300?n:null;}
  if(field==='label'){q.label=e.target.value.slice(0,150);const p=parseQuestionLabel(q.label);if(p.section)q.section=p.section;if(p.number)q.number=p.number;}
  if(field!=='label')relabel(q,old);
  let extra=0;
  // The rest of a run that OCR put under the wrong section: offer to move it along.
  if(q.section!==old.section){
   const i=draft.questions.indexOf(q),run=[];let prev=q.number;
   for(let j=i+1;j<draft.questions.length;j++){const r=draft.questions[j];if(r.section!==old.section)break;if(r.number&&prev&&r.number!==prev+1)break;run.push(r);prev=r.number||prev;}
   if(run.length&&await ask(`이어지는 ${run.length}문항도 ${sectionName(q.section)}(으)로 바꿀까요?`,`목록에서 바로 뒤에 이어지는 ${sectionName(old.section)} ${run.map(r=>r.number?r.number+'번':'번호 없음').join(', ')}도 같은 영역으로 옮깁니다.`,'함께 변경')){
    for(const r of run){const o={section:r.section,number:r.number};r.section=q.section;relabel(r,o);}extra=run.length;
   }
  }
  const dup=q.number&&draft.questions.filter(r=>r!==q&&r.section===q.section&&r.number===q.number).length;
  const moved=reorder(q.id);
  if(!moved)renderList();
  saveDraft();
  status(`${q.label}${extra?` 외 ${extra}문항`:''} 수정${moved?' · 영역·번호 순서로 다시 배치했습니다':''}.${dup?` ⚠ ${sectionName(q.section)} ${q.number}번이 이미 있어 다음 회차로 배치됩니다. 번호를 확인하세요.`:''}`);
 }
 ...
 function pushPart(part,{section=$('cropSection').value,number=null,prefix=shared}={}){const q={id:uid(),label:questionLabel(section,number,draft.questions.length),number,section,key:null,source:pdfName,parts:prefix?[prefix,part]:[part]};draft.questions.push(q);selectedId=q.id;return q;}
 function addSelection(mode){if(!pdf||busy)return;if(!selection||selection.w<.005||selection.h<.005)return toast('PDF에서 문제 전체를 드래그해 선택하세요.');if(mode==='add'&&draft.questions.length>=500)return toast('최대 500문항까지 준비할 수 있습니다.');const q=draft.questions.find(q=>q.id===selectedId);if(mode!=='add'&&!q)return toast('오른쪽 문항 미리 보기를 눌러 대상 문항을 먼저 선택하세요.');if(mode==='append'&&q.parts.length>=20)return toast('한 문항은 최대 20조각까지 이어 붙일 수 있습니다.');checkpoint();const part=cropPart(selection);if(mode==='add'){const n=Math.round(Number($('cropNumber').value))||null;const added=pushPart(part,{number:n>=1&&n<=300?n:null});if(added.number){$('cropNumber').value=added.number+1;reorder(added.id);}}else if(mode==='append')q.parts.push(part);else q.parts=shared?[shared,part]:[part];selection=null;drawOverlays();renderList();saveDraft();status(mode==='append'?'선택 문항에 이미지를 이어 붙였습니다.':mode==='replace'?'선택 문항의 이미지를 교체했습니다.':`문항을 추가했습니다${$('cropNumber').value?` · 다음 번호 ${$('cropNumber').value}번`:' · 번호를 입력하면 순서대로 자동 배치됩니다'}. 다음 영역을 드래그하세요.`);}
